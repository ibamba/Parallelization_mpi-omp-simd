...
*/
//PARTIE NON PARALLELISABLE : à cause de la récursivité
...
int main(int argc, char **argv)
{
	...
	int local_h, cur_h; /*Hauteur locale et hauteur courant dans l'image intégrale (valeur de i dans l'image intégrale)*/

	double *image, *local_image;

	/* Variables mpi */
	int rank, np, root=0; // rang, nbre processus, tag, processus root
	...
	fprintf(stderr, "%d commence\n", rank);

	/* Allocation du tampon image */
	local_image = malloc(3 * w * local_h * sizeof(*local_image));
	if (local_image == NULL) {
	...
	for (int i = 0; i < local_h; i++) {
		cur_h=i+(np-1-rank)*local_h;
 		unsigned short PRNG_state[3] = {0, 0, cur_h*cur_h*cur_h}; /**/
		for (unsigned short j = 0; j < w; j++) {
			...
			copy(pixel_radiance, local_image + 3 * ((local_h - 1 - i) * w + j)); // <-- retournement vertical /**/
		}
	}

	/* Regroupement des données calculées */
	MPI_Gather(local_image, 3*local_h*w, MPI_DOUBLE, image, 3*local_h*w, MPI_DOUBLE, root, MPI_COMM_WORLD);

	free(local_image);

	/* fin du chronometrage */
	fin = wtime();
	fprintf( stderr, "\nProcess %d terminé : Temps total de calcul : %g sec\n", rank, fin - debut);
	...
	if(rank==root){
		...
		sprintf(nom_sortie, "%s/stat-mpi-image.ppm", nom_rep);
